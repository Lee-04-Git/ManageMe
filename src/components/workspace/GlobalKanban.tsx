...
import { Task, Workspace, currentUser } from "@/data/workspaceData";
import {
  GripVertical,
  MoreVertical,
  Calendar,
...
export default function GlobalKanban({
  ...
  // Get all tasks from all workspaces where user is a member
  const allTasks = workspaces
    .filter((ws) => ws.members.includes(currentUser.id))
    .flatMap((ws) =>
      ws.channels
        .filter((ch) => ch.members.includes(currentUser.id))
        .flatMap((ch) => ch.tasks)
    );
  ...
  const getTasksByStatus = (status: Task["status"]) => {
    return localTasks.filter((task) => {
      ...
      const matchesWorkspace =
        !filterWorkspace || task.workspaceId === filterWorkspace;
      return matchesStatus && matchesSearch && matchesWorkspace;
    });
  };
  ...
  return (
    <div className="h-full flex flex-col bg-[#1a1d23]">
      {/* Global Kanban Header */}
      <div className="bg-[#1e2128] border-b border-gray-800 px-6 py-4">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-white font-bold text-xl mb-1">All My Tasks</h2>
        ...
        <div className="flex items-center gap-6 mb-4">
          ...
          <div>
            ...
            <p className="text-green-400 font-semibold text-lg">{stats.done}</p>
          </div>
          <div className="w-px h-10 bg-gray-800"></div>
          <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full bg-[#2a2d35] text-white placeholder-gray-500 pl-10 pr-4 py-2 rounded-lg text-sm border border-gray-700 focus:border-[#ff6b6b] focus:outline-none transition-colors"
            />
          </div>
          ...
          <select
            ...
            className="bg-[#2a2d35] text-white px-4 py-2 rounded-lg text-sm border border-gray-700 focus:border-[#ff6b6b] focus:outline-none transition-colors"
          >
...
interface GlobalKanbanColumnProps {
  column: Column;
  tasks: Task[];
  workspaces: Workspace[];
      ...
      >
        ...
        {tasks.length === 0 && !isDragging && (
          ...
          >
            ...
            >
              <div
                className={`w-6 h-6 rounded-full border-2 border-dashed ${column.color.replace(
                  "bg-",
                  "border-"
                )}`}
              ></div>
...
function GlobalTaskCard({
  ...
  const workspace = workspaces.find((ws) => ws.id === task.workspaceId);
  const channel = workspace?.channels.find((ch) => ch.id === task.channelId);
    ...
    >
      ...
      <div className="p-4 pt-5">
        ...
        <div className="flex items-center gap-2 mb-3">
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-[#1a1d23] border border-gray-800">
          ...
          {channel && (
            <div className="px-2 py-1 rounded-md bg-[#1a1d23] border border-gray-800">
              <span className="text-gray-400 text-xs font-medium">
                #{channel.name}
              </span>
            </div>
